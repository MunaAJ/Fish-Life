...
* is filled with WATER.  The border around the perimeter of the landscape      * 
* array (top, bottom, left, right) is then overwritten with ROCK.              *
* <p>																		   *
* Random rocks are placed in the pond until the number of rocks (in addition   * 
* to those in the border) reaches numRocks. 								   *
* <p>																		   *
* The "plants" ArrayList is instantiated. Randomly placed Plant objects are    *
* put into the List.  Their positions are chosen so that they are never above  *
* rocks or in the same position as another plant. Plants are generated in this *
* way until the list reaches size numPlants.                                   *
* <p>																		   *
...
*******************************************************************************/
	...
	public Model(int numRows, int numCols, int numRocks, int numFish, int 
				numPlants) {
...
* created If there are no available regions nearby, the plant will not explode.* 
*******************************************************************************/
		...
		while(i.hasNext()) {
			...
			* the coordinates of the plant that is exploding. */
				...
				/* Add little plants to the list -- iterator is now broken! */
					 ...
					 * is no longer useful.  Start iterating from 
					 * the beginning. */
					i = plants.iterator();
				}	
			}
	...
	public void fishExplosions() {
		...
		while(i.hasNext()) {
			...
			if (curr.getSize() > Fish.MAX_FISH_SIZE) {
				...
				boolean[] freeSpace = new boolean[8];  /* true if just water 
														in that region*/
 ...
 * If so, returns false; if it is just water, returns true. */
	public boolean isSpaceAvailable(int r, int c) {
		...
		for(Plant firstPlant : plants) {
			if (firstPlant.getRow() == r && firstPlant.getCol() == c) {
				return false;
			}
		}
		for(Fish firstFish : fish) {
			if (firstFish.getRow() == r && firstFish.getCol() == c) {
				return false;
			}
		}
		return true;
...
*******************************************************************************/
	...
	public void shrinkFish() {
		for (Fish aliveFish : fish) {
			if (aliveFish.isAlive()) {
				aliveFish.shrink();
			}
		}
	}
...
*******************************************************************************/
	...
	public void growPlants() {
		for (Plant alivePlants : plants) {
			if (alivePlants.isAlive()) {
				alivePlants.grow();
			}
		}
	}
...
/******************************************************************************* 
* This method is used to check if the direction in which a fish is heading     *
* has a rock.  							                       				   *
*******************************************************************************/
	
	private boolean hittingARock(Fish p) {
		if (p.getDirection() == Fish.UP && 
				landscape[p.getRow() - 1]
						[p.getCol()] == ROCK) {
			return true;
		}
		if (p.getDirection() == Fish.DOWN && 
				landscape[p.getRow() + 1]
						[p.getCol()] == ROCK) {
			return true;
		}
		if (p.getDirection() == Fish.LEFT && 
				landscape[p.getRow()]
						[p.getCol() - 1] == ROCK) {
			return true;
		}
		if (p.getDirection() == Fish.RIGHT && 
				landscape[p.getRow()]
						[p.getCol() + 1] == ROCK) {
			return true;
		}
...
*******************************************************************************/
	...
	public void turnFish() {
		for (Fish aliveFish : fish) {
			if (aliveFish.isAlive() == true && 
					fishIsSurroundedByRocks(aliveFish) == false) {
				if (aliveFish.getDirection() != Fish.UP && 
						aliveFish.getDirection() != Fish.DOWN &&  
						aliveFish.getDirection() != Fish.LEFT && 
						aliveFish.getDirection() != Fish.RIGHT) {
					throw new IllegalFishDirectionException(aliveFish.
							getDirection());
				}
				while (hittingARock(aliveFish)) {
					aliveFish.setRandomDirection();
				}
			}
		}
	}
...
*******************************************************************************/
	...
	public void moveFish() { 
		for (Fish aliveFish : fish) {
			if (aliveFish.isAlive()) {
				if (fishIsSurroundedByRocks(aliveFish) == false) {
					aliveFish.move();
					for (Plant alivePlant : plants) {
						if (alivePlant.isAlive()) {
							if (aliveFish.getRow() == alivePlant.getRow() && 
									aliveFish.getCol() == alivePlant.getCol()) {
								fishEatPlant(aliveFish, alivePlant);
							}
						}
					}
				}
			}
			for (Fish secondFish : fish) {
				if(secondFish.isAlive() == true && aliveFish != secondFish){
					if (aliveFish.getRow() == secondFish.getRow() && 
							aliveFish.getCol() == secondFish.getCol()) {
						aliveFish.fight(secondFish);
					}
				}
			}
		}
...
*******************************************************************************/
	...
	public void addPlant(Plant p) {
		if (landscape[p.getRow()][p.getCol()] == ROCK) {
			...
			for (Plant anotherPlant : plants) {
				if (anotherPlant.getRow() == p.getRow() && anotherPlant.getCol() 
						== p.getCol()) {
					throw new IllegalPlantPositionException
...
* added to the list.  Instead throws an IllegalFishPositionException, passing  *
...
*******************************************************************************/
	...
	public void addFish(Fish f){
		if (landscape[f.getRow()][f.getCol()] == ROCK) {
			...
			for (Fish anotherFish : fish) {
				if (anotherFish.getRow() == f.getRow() && anotherFish.getCol() 
						== f.getCol()) {
					throw new IllegalFishPositionException
